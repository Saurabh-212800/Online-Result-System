...
<body>
	...
	<div id="allrecords">
		<form action="" method="POST" class="student">
					...
					if (isset($_POST['Confirm'])) {
						...
						if ($row=mysqli_fetch_array($runquery1)) {
							if ($stud_id!=$row["stud_id"]) {
								if ($roll!=$row["roll"]) {
									mysqli_query($conn,"UPDATE student SET stud_id='$stud_id',name='$name',roll='$roll',class='$class',sem='$sem' WHERE stud_id='$stud_id'");

									mysqli_query($conn,"UPDATE subject SET stud_id='$stud_id',sem='$sem',S1='$S1',S2='$S2',S3='$S3',S4='$S4',S5='$S5' WHERE stud_id='$stud_id'");

									mysqli_query($conn,"UPDATE result SET stud_id='$stud_id',roll='$roll',sem='$sem',s1='$s1',s2='$s2',s3='$s3',s4='$s4',s5='$s5' WHERE stud_id='$stud_id'");

									mysqli_query($conn,"UPDATE class SET stud_id='$stud_id',class='$class',sem='$sem' WHERE stud_id = '$stud_id'");
			...
			<h2>Upadate Data</h2>
			<div>
				<input type="txt" name="stud_id" placeholder="Enter Student Id" required autocomplete="off">
			</div>
			<div>
				<input type="txt" name="name" placeholder="Enter Name" required autocomplete="off">
			</div>
			<div>
				<input type="text" name="roll" placeholder="Enter Roll Number" required autocomplete="off">
			</div>
			<div>
				<input type="text" name="class" placeholder="Enter Class" required autocomplete="off">
			</div>
